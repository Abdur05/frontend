...
@Component({
  selector: 'app-add-tax-dec-employee',
  templateUrl: './add-tax-dec-employee.component.html',
  styleUrls: ['./add-tax-dec-employee.component.css']
})
export class AddTaxDecEmployeeComponent {
  ...
  createFormdata() {
    ...
    this.taxDeclarationFormGroup = this.fb.group({
      fiscal_year: [this.fiscalYear, Validators.required],
  ...
  createEmployeeGroup() {
    return this.fb.group({
        ...
        declared_amount: ['', [Validators.required, Validators.min(1)]],
    });
  }
  ...
  async submitData() {
    try {
      ...
      if (this.taxDeclarationFormGroup.invalid)
        return
      const result: any = await this.taxDecEmployeeSer.createTaxDeclarationEmployee(this.taxDeclarationFormGroup.value)
  ...
  async getAllTaxDeclarationDetail() {
    try {
      ...
      if (result.status === true) {
        ...
        this.taxDecDetail.map((el: any) => el.disable = false)
      }
    } catch (error: any) {
      this._snackBar.open(error.error.message, '', {
        duration: 5 * 1000, horizontalPosition: 'center',
